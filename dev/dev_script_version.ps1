...
# TO VISUALIZE JOB RUN IN GRAPH EVEN IF THEY ARE SHORT
$SecondsFromStart = (GetUnixTimeUTC(Get-Date)) - $START_TIME_UTC_EPOCH
if ($SecondsFromStart -le 3600) {
    $LAST_SESSION_RESULT_CODE = -1
}

# DISTINGUISH JOB RUN BEING FULL BACKUP OR A FULL SYNTHETIC BACKUP
if ($LAST_SESSION_RESULT_CODE -eq -1) {
    if ($LastSession.IsFullMode) {
...
# TO VISUALIZE SHORT JOB RUNS IN GRAPH
