...
# CHECKS IF LAST SESSION WAS FULL BACKUP OR FULL SYNTHETIC
function WasLastRunFullActiveOrFullSynt($aLastSess) {
    if ($aLastSess.IsFullMode) { return $True }

    $LastSessTasks = Get-VBRTaskSession -Session $aLastSess
    $LastTasksLogs = $LastSessTasks.Logger.GetLog().UpdatedRecords.Title
    $SyntText = 'Synthetic full backup created successfully'
    return ($LastTasksLogs -Contains $SyntText)
}
...
# TO BETTER VISUALIZE JOB RUN IN GRAPH EVEN IF THEY ARE SHORT
# AND TO DISTINGUISH RUN BEING FULL BACKUP OR A FULL SYNTHETIC BACKUP
$SecondsFromStart = (GetUnixTimeUTC(Get-Date)) - $START_TIME_UTC_EPOCH
if ($SecondsFromStart -le 3600) {
    ...
    if (WasLastRunFullActiveOrFullSynt $LastSession) {
